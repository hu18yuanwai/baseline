			...
			for( std::list<Track>::iterator iTrack = tracks.begin(); iTrack != tracks.end(); )
			{
				...
				if(iTrack->index >= trackInfo.length) {
					...
					if(( IsValid(trajectory, mean_x, mean_y, var_x, var_y, length,saliency, averageSaliency, threshold_s) == 1 ) && IsCameraMotion(displacement))
					{
						...
						//------------------by hu-------------
						float saliency_out =0.0;
						for(int i_s = 0; i_s<saliency.size();i_s++){
							//cout<<saliency[i]<<" ";
							saliency_out+=saliency[i];
						}
						//cout<<endl;
						saliency_out/=saliency.size();
						//------------------end----------------
						printf("%d\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t", frame_num, mean_x, mean_y, var_x, var_y, length, fscales[iScale],saliency_out);
