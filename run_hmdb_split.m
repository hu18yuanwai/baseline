function des_accs = run_hmdb_split(split,dataset)
%run_split:
% Example:
...
%    normalize: choose one method from {'Power-L2','Power-Intra-L2'}.
    ...
    for i = 1 : numel(descriptorType)
        ...
        if ~exist( fullfile('/home/hu/remote/Data/HMDB51/feats/train',sprintf('%s_%s_%d_Kern.mat',descriptorType{i},encode_method,split)),'file')
            ...
            feature = normalize(feature,'Power-L2',gmmSize);
            ...
            %TrainData = feature(trn_indx,:);
            %template = get_ran_template(TrainData,trainLabels);
            %feature = ran_test(template,feature);
            TrainData = feature(trn_indx,:);
        ...
        if i==1
            ...
            tr_kern_sum = TrainData_Kern+tr_kern_sum;
            ts_kern_sum = TestData_Kern+ts_kern_sum;
        end
